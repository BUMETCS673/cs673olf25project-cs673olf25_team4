...
# Deploys BeatMap application to test server (testbeatmap.com) with Let's Encrypt SSL
...
DOMAIN="${STAGING_DOMAIN:-testbeatmap.com}"
LETSENCRYPT_DIR="/etc/letsencrypt/live/$DOMAIN"
BACKUP_DIR="$SCRIPT_DIR/backups/staging_$(date +%Y%m%d_%H%M%S)"
...
banner() {
    echo -e "${CYAN}"
    echo "╔════════════════════════════════════════════════╗"
    echo "║   BeatMap Staging Deployment (HTTPS)          ║"
    echo "║          Test Server: $DOMAIN         ║"
    echo "╚════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
check_permissions() {
    if [[ $EUID -ne 0 ]]; then
        ...
        info "Usage: sudo bash deploy-staging.sh"
        exit 1
    fi
}
...
staging_confirmation() {
    warn "⚠️  STAGING DEPLOYMENT (Test Server) ⚠️"
    echo ""
    echo "You are about to deploy to STAGING environment:"
    echo "  Domain: $DOMAIN"
    echo "  Environment: Staging/Test"
    echo "  SSL: Let's Encrypt (Production certs)"
    echo "  IP: 18.224.92.5"
    echo ""
    ...
    read -p "Continue with staging deployment? (y/N): " CONFIRMATION
    if [[ "$CONFIRMATION" != "y" ]] && [[ "$CONFIRMATION" != "Y" ]]; then
        warn "Deployment cancelled by user"
        exit 0
    fi

    success "Staging deployment confirmed"
}
...
verify_dns() {
    ...
    # Expected IP for testbeatmap.com
    EXPECTED_IP="18.224.92.5"
    ...
    if [[ -z "$DNS_IP" ]]; then
        ...
        error "Please configure DNS before deploying to staging"
        exit 1
    fi
    ...
    if [[ "$DNS_IP" != "$EXPECTED_IP" ]]; then
        warn "DNS IP ($DNS_IP) does not match expected IP ($EXPECTED_IP)"
...
check_ssl_certificates() {
    ...
    if [[ ! -f "$LETSENCRYPT_DIR/fullchain.pem" ]] || [[ ! -f "$LETSENCRYPT_DIR/privkey.pem" ]]; then
        ...
        if [[ -f "$SSL_DIR/setup-testbeatmap-ssl.sh" ]]; then
            bash "$SSL_DIR/setup-testbeatmap-ssl.sh"
            ...
            error "SSL setup script not found at $SSL_DIR/setup-testbeatmap-ssl.sh"
            exit 1
        fi
    else
...
deploy_containers() {
    step "Building and starting staging containers..."
    ...
    # Start containers
    info "Starting containers in staging mode..."
    if command -v docker compose &> /dev/null; then
...
display_info() {
    ...
    echo ""
    echo -e "${YELLOW}Note:${NC} This is a staging environment for testing before production deployment"
    echo ""
}
