...
class ConcertsService:
    def __init__(self):
        ...
        self.router.add_api_route("/search", self.search, methods=["GET"], tags=["concerts"])

        # Provider URLs from env
        self.providers = {
            "jambase": os.getenv("JAMBASE_PROVIDER_URL", "http://jambase_provider:8000"),
            "ticketmaster": os.getenv("TM_PROVIDER_URL", "http://ticketmaster_provider:8000"),
        }
    ...
    async def search(
        ...
        provider: str = Query(None, description="Provider to search (jambase, ticketmaster)"),
