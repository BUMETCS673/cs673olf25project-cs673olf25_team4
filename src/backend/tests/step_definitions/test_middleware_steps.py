...
def cors_origins_include_localhost(middleware_context):
    """Given CORS origins include localhost."""
    middleware_context['test_env']['CORS_ORIGINS'] = 'http://localhost:3000,https://localhost:3000'


@given('CORS origins are restricted to secure domains')
def cors_origins_restricted_secure(middleware_context):
    """Given CORS origins are restricted to secure domains."""
    middleware_context['test_env']['CORS_ORIGINS'] = 'https://beatmap.live,https://api.beatmap.live'
...
def cors_headers_reject_origin(middleware_context):
    ...
    assert access_control_origin is None or 'malicious-site.com' not in access_control_origin
...
def log_includes_request_details(middleware_context):
    ...
    mock_logger = middleware_context.get('mock_logger')
    if mock_logger:
        # Check that logging was called (actual log format verification would be complex)
        assert mock_logger.info.called or mock_logger.debug.called
...
def sensitive_info_redacted_from_logs(middleware_context):
    ...
    mock_logger = middleware_context.get('mock_logger')
    if mock_logger:
        # Ensure logging is happening, redaction logic would need specific implementation
        assert mock_logger.info.called or mock_logger.debug.called
