...
**BeatMap uses HTTPS for ALL communication**, not just the frontend:

1. **Frontend ↔ User**: Browser connects to frontend via HTTPS
2. **Frontend ↔ Backend**: Frontend API calls to backend via HTTPS
3. **Backend ↔ Providers**: Backend communicates with all provider services (Ticketmaster, Jambase, Groq) via HTTPS
4. **All Internal Services**: Every microservice uses HTTPS for inter-service communication

### SSL Verification by Environment

**Development**:
- Uses self-signed certificates (generated by `generate-dev-certs.sh`)
- SSL verification is **disabled** to allow self-signed certificates
- All services still use HTTPS URLs, but don't validate certificates
- This is configured via the `ENVIRONMENT=development` variable

**Production/Staging**:
- Uses Let's Encrypt certificates (trusted CA)
- SSL verification is **enabled** for full security
- All certificates are properly validated
- This is configured via `ENVIRONMENT=production` or `ENVIRONMENT=staging`

### How It Works

The backend automatically adjusts SSL verification based on environment:

```python
# In development: verify=False (allows self-signed certs)
# In production: verify=True (validates certificates)
...
| Environment | Certificate Type | SSL Verification | Location |
|------------|------------------|------------------|----------|
| Development | Self-signed | Disabled | `ssl/dev/` |
| Test (testbeatmap.com) | Let's Encrypt | Enabled | `/etc/letsencrypt/live/testbeatmap.com/` |
| Production (beatmap.live) | Let's Encrypt | Enabled | `/etc/letsencrypt/live/beatmap.live/` |

## 📋 Scripts Overview

### 🔧 Certificate Generation Scripts

#### `ensure-dev-certs.sh` ⭐ Recommended
Automatically checks for and generates development certificates if needed.
...
- Validates certificate expiration (warns if < 24 hours)
- Automatically generates new certificates if missing or invalid
- Idempotent - safe to run multiple times
- Used automatically by the development startup script

**When to use:**
- Automatically called by `start-dev.sh`
- Run manually if you need to regenerate certificates
- Part of the automated development workflow

#### `generate-dev-certs.sh`
Generates self-signed certificates for local development (used by ensure-dev-certs.sh).
...
- Generates Diffie-Hellman parameters for enhanced security
